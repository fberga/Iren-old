...
namespace Iren.ToolsExcel.Base
{
    public abstract class ASheet
    {
        ...
        public abstract void AggiornaDateTitoli2();
        public abstract void AggiornaGrafici();
        public abstract void AggiornaGrafici2();
    ...
    public class Sheet : ASheet, IDisposable
    {
        ...
        protected void InitBarraNavigazione()
        {
            ...
            if (Struct.tipoVisualizzazione == "O")
            {
                ...
                CicloGiorni((oreGiorno, suffissoData, giorno) =>
                {
                    ...
                    InsertOre2(_struttura.rigaBlock - 1, colonnaInizio, giorno, oreGiorno);
        ...
        protected void InitBloccoEntita(DataRowView entita)
        {
            ...
            InsertTitoloVerticale2(entita["DesEntitaBreve"]);
            InsertGrafici2();
        ...
        protected virtual void CreaNomiCelle2(object siglaEntita)
        {
            ...
            if (Struct.tipoVisualizzazione == "O")
                _newNomiDefiniti.AddName(_rigaAttiva, siglaEntita, "T");
            else
                _newNomiDefiniti.AddName(_rigaAttiva, suffissoData, "T");
        ...
        protected virtual void InsertTitoloEntita2(object siglaEntita, object desEntita)
        {
            ...
            {
                int col = _newNomiDefiniti.GetColFromDate(suffissoData);
                int row = _newNomiDefiniti.GetRowByName(Struct.tipoVisualizzazione == "O" ? siglaEntita : suffissoData, "T");
        ...
        protected virtual void InsertOre2(object siglaEntita)
        {
            if (Struct.tipoVisualizzazione == "V")
            {
                ...
                int row = _newNomiDefiniti.GetRowByName(Date.GetSuffissoData(_dataInizio), "T") + 1;

                InsertOre2(row, col, _dataInizio, 25);
            }
        }
        private void InsertOre2(int rigaAttiva, int colonnaInizio, DateTime giorno, int oreGiorno)
        {
        ...
        protected virtual void InsertTitoloVerticale2(object desEntita)
        {
            DataView informazioni = DataBase.LocalDB.Tables[DataBase.Tab.ENTITAINFORMAZIONE].DefaultView;

            object siglaEntitaRif = informazioni[0]["SiglaEntitaRif"] is DBNull ? informazioni[0]["SiglaEntita"] : informazioni[0]["SiglaEntitaRif"];
            int row = _newNomiDefiniti.GetRowByName(siglaEntitaRif, informazioni[0]["SiglaInformazione"], Struct.tipoVisualizzazione == "O" ? "" : Date.GetSuffissoData(_dataInizio));
        ...
        protected virtual void InsertInformazioniEntita2()
        {
            ...
            Excel.Range rngRow = _ws.Range[GetRange(row, col - _visParametro, informazioni.Count - 1, colOffset + _visParametro - 1)];
            Excel.Range rngInfo = _ws.Range[GetRange(row, col - _visParametro, informazioni.Count - 1, 1)];
            Excel.Range rngData = _ws.Range[GetRange(row, col, informazioni.Count - 1, colOffset - 1)];
        ...
        protected virtual void InsertFormuleValoriDefault()
        {
            ...
            foreach (DataRowView info in informazioni)
            {
                ...
                int col = _newNomiDefiniti.GetFirstCol() + (_struttura.visData0H24 && info["Data0H24"].Equals("0") ? 1 : 0);
                int realColOffset = colOffset - (_struttura.visData0H24 && info["Data0H24"].Equals("0") ? 1 : 0);
                object siglaEntita = info["SiglaEntitaRif"] is DBNull ? info["SiglaEntita"] : info["SiglaEntitaRif"];
                int row = _newNomiDefiniti.GetRowByName(siglaEntita, info["SiglaInformazione"], Struct.tipoVisualizzazione == "O" ? "" : Date.GetSuffissoData(_dataInizio));
                Excel.Range rngData = _ws.Range[GetRange(row, col, 0, realColOffset - 1)];
                ...
                else if (info["FormulaInCella"].Equals("1"))
                {
                    int deltaNeg;
                    int deltaPos;
                    string formula = "=" + PreparaFormula2(info, "DATA0", "DATA1", 24, out deltaNeg, out deltaPos);

                    if (info["SiglaTipologiaInformazione"].Equals("OTTIMO"))
                    {
                        string rng = GetRange(row, col + 1, 0, realColOffset - 1);
                        rngData.Cells[1].Formula = "=SUM(" + GetRange(row, col + 1, 0, realColOffset - 1) + ")";
                        deltaNeg = 1;
                    }
                    _ws.Range[GetRange(row, col + deltaNeg, 0, realColOffset - deltaNeg - deltaPos - 1)].Formula = formula;
        ...
        protected virtual void InsertParametri2()
        {
            DataView informazioni = DataBase.LocalDB.Tables[DataBase.Tab.ENTITAINFORMAZIONE].DefaultView;
            ...
            foreach (DataRowView info in informazioni)
            {
                ...
                {
                    int col = _newNomiDefiniti.GetColFromDate(suffissoData);
                    int row = _newNomiDefiniti.GetRowByName(siglaEntita, info["SiglaInformazione"], Struct.tipoVisualizzazione == "O" ? "" : Date.GetSuffissoData(suffissoData));
                    int colOffset = Struct.tipoVisualizzazione == "O" ? _newNomiDefiniti.GetDayOffset(suffissoData) : _newNomiDefiniti.GetColOffset();

                    Excel.Range rngData = _ws.Range[GetRange(row, col, 0, colOffset - 1)];
                    ...
                    if (parametriD.Count > 0)
                        rngData.Value = parametriD[0]["Valore"];
                    else
                    {
            ...
            foreach (DataRowView info in informazioni)
            {
                ...
                foreach (DataRowView format in formattazione)
                {
                    ...
                    if (format["NomeCella"] != DBNull.Value)
                    {
                        ...
                        string address = GetRange(refRow, col);
                        string formula = "";
        ...
        protected virtual void InsertGrafici2()
        {
            DataView grafici = DataBase.LocalDB.Tables[DataBase.Tab.ENTITAGRAFICO].DefaultView;
            DataView graficiInfo = DataBase.LocalDB.Tables[DataBase.Tab.ENTITAGRAFICOINFORMAZIONE].DefaultView;
            ...
            foreach (DataRowView grafico in grafici)
            {
                ...
                Excel.Range rngGrafico = _ws.Range[GetRange(row, col, 0, colOffset - 1)];
                rngGrafico.Merge();
                rngGrafico.Style = "chartsBarStyle";
                rngGrafico.RowHeight = 200;
                Excel.Chart chart = _ws.ChartObjects().Add(rngGrafico.Left, rngGrafico.Top + 1, rngGrafico.Width, rngGrafico.Height - 2).Chart;
                ...
                {
                    int rowDati = _newNomiDefiniti.GetRowByName(grafico["SiglaEntita"], info["SiglaInformazione"], Struct.tipoVisualizzazione == "O" ? "" : Date.GetSuffissoData(_dataInizio));
                    Excel.Range datiGrafico = _ws.Range[GetRange(rowDati, col, 0, colOffset)];
        ...
        public override void AggiornaGrafici2()
        {
            ...
            foreach (Excel.ChartObject chart in charts)
            {
                ...
                Excel.Range rng = _ws.Range[GetRange(row, col)];
                AggiornaGrafici2(chart.Chart, rng.MergeArea);
                chart.Chart.Refresh();
            }
        }
        private void AggiornaGrafici2(Excel.Chart chart, Excel.Range rigaGrafico)
        {
            ...
            try
            {
                ...
                if (Struct.tipoVisualizzazione == "O")
                {
                    foreach (DataRowView entita in categoriaEntita)
                    {
                        ...
                        CaricaInformazioniEntita2(datiApplicazione);
                        if (all)
                        {
                            ...
                            CaricaCommentiEntita2(insertManuali);
                        }
                    }
                }
                else
        ...
        private void CaricaInformazioniEntita2(DataView datiApplicazione)
        {
            foreach (DataRowView dato in datiApplicazione)
            {
                ...
                {
                    //non serve gestire il caso della visualizzazione verticale: DATA0.H24 non esiste lì
                    int col = _newNomiDefiniti.GetColFromDate("DATA0", "H24");
                    int row = _newNomiDefiniti.GetRowByName(dato["SiglaEntita"], dato["SiglaInformazione"]);
                    _ws.Range[GetRange(row,col)].Value = dato["H24"];
                }
                else
                {
                    ...
                    _ws.Range[GetRange(row, col, 0, dayOffset - 1)].Value = o.ToArray();
                }
            }
        }
        private void CaricaCommentiEntita2(DataView insertManuali)
        {
            foreach (DataRowView commento in insertManuali)
            {
                ...
                int col = Struct.tipoVisualizzazione == "O" ? _newNomiDefiniti.GetColFromDate(giorno) : _newNomiDefiniti.GetFirstCol();
                int row = _newNomiDefiniti.GetRowByName(commento["SiglaEntita"], commento["SiglaInformazione"], Struct.tipoVisualizzazione == "O" ? "" : Date.GetSuffissoData(commento["Data"].ToString()));
            ...
            foreach (DataRowView entita in dvCE)
            {
                ...
                if (informazioni.Count > 0)
                {
                    ...
                    foreach (DataRowView info in informazioni)
                    {
                        ...
                        string formula = "=" + PreparaFormula2(info, suffissoDataPrec, suffissoData, oreDataPrec, out deltaNeg, out deltaPos);
        ...
        protected string PreparaFormula2(DataRowView info, string suffissoDataPrec, string suffissoData, int oreDataPrec, out int deltaNeg, out int deltaPos)
        {
            if (info["Formula"] != DBNull.Value || info["Funzione"] != DBNull.Value)
            {
                string formula = info["Formula"] is DBNull ? info["Funzione"].ToString() : info["Formula"].ToString();
                ...
                formula = Regex.Replace(formula, @"%P\d+(E\d+)?%", delegate(Match m)
                {
                    ...
                    if (parametroEntita.Length > 1)
                    {
                        ...
                        DataView categoriaEntita = DataBase.LocalDB.Tables[DataBase.Tab.CATEGORIAENTITA].DefaultView;
                        categoriaEntita.RowFilter = "Gerarchia = '" + info["SiglaEntita"] + "' AND Riferimento = " + eRif;
                        nome = NewDefinedNames.GetName(categoriaEntita[0]["SiglaEntita"], parametri[n - 1]);
                    }
                    else
                        nome = NewDefinedNames.GetName(info["SiglaEntita"], parametri[n - 1]);
                    ...
                    if (Struct.tipoVisualizzazione == "O")
                    {
                        row = _newNomiDefiniti.GetRowByName(nome);
                        col = _newNomiDefiniti.GetColFromDate(suffData, suffOra);
                    }
                    else
                    {
                        row = _newNomiDefiniti.GetRowByName(nome + Simboli.UNION + suffData);
                        col = _newNomiDefiniti.GetColFromDate("DATA1", suffOra);
                    }

                    return R1C1toA1(row, col);
                }, RegexOptions.IgnoreCase);
                return formula;
            }
        ...
        private void CancellaDati2()
        {
            CancellaDati2(DataBase.DataAttiva, true);
        }
        private void CancellaDati2(DateTime giorno, bool all = false)
        {
            DataView categoriaEntita = DataBase.LocalDB.Tables[DataBase.Tab.CATEGORIAENTITA].DefaultView;
            DataView entitaProprieta = DataBase.LocalDB.Tables[DataBase.Tab.ENTITAPROPRIETA].DefaultView;
            categoriaEntita.RowFilter = "SiglaCategoria = '" + _siglaCategoria + "'"; // AND (Gerarchia = '' OR Gerarchia IS NULL )";
        ...
        public override void AggiornaDateTitoli2()
        {
            if (Struct.tipoVisualizzazione == "O")
            {
                ...
                for (int i = 0; i < _newNomiDefiniti.DaySuffx.Length; i++)
                {
                    if (_newNomiDefiniti.DaySuffx[i] != "DATA0")
                    {
                        ...
                        _ws.Range[GetRange(row, col)].Value = Date.GetDataFromSuffisso(_newNomiDefiniti.DaySuffx[i]);
                    }
                }
            }
            else
            {
                for (int i = 0; i < Struct.intervalloGiorni; i++)
                {
                    ...
                    int col = _newNomiDefiniti.GetFirstCol();
                    _ws.Range[GetRange(row, col)].Value = giorno;
        ...
        protected void CaricaParametri2()
        {
            DataView entitaProprieta = DataBase.LocalDB.Tables[DataBase.Tab.ENTITAPROPRIETA].DefaultView;
            DataView categoriaEntita = DataBase.LocalDB.Tables[DataBase.Tab.CATEGORIAENTITA].DefaultView;
            DataView informazioni = DataBase.LocalDB.Tables[DataBase.Tab.ENTITAINFORMAZIONE].DefaultView;
